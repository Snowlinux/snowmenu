...
class pluginclass( object ):
    ...
    @print_timing
    def __init__( self, snowMenuWin, toggleButton, de ):
        self.snowMenuWin = snowMenuWin
        ...
        self.snowMenuWin.SetHeadingStyle( [self.wTree.get_widget("label6"), self.wTree.get_widget("label2")] )
        ...
        self.itemstocolor = [ self.wTree.get_widget( "viewport1" ), self.wTree.get_widget( "viewport2" ), self.wTree.get_widget( "viewport3" ), self.wTree.get_widget( "hpaned1" ) ]
        ...
        self.gconfHandlers = []
        # Gconf stuff
        self.gconf = EasyGConf( "/apps/snowMenu/plugins/applications/" )
    ...
    def refresh_apt_cache(self):
        if self.useAPT:
            os.system("mkdir -p %s/.snowlinux/snowMenu/" % home)
            os.system("/usr/lib/snowlinux/snowMenu/plugins/get_apt_cache.py > %s/.snowlinux/snowMenu/apt.cache &" % home)
    ...
    def get_panel(self):
        ...
        for applet in appletidlist:
            ...
            if bonobo_id == "OAFIID:MATE_snowMenu":
    ...
    def RegenPlugin( self, *args, **kargs ):
        ...
        if oldcategories_mouse_over == self.categories_mouse_over and \
                                    ...
                                    oldtotalrecent == self.totalrecent and \
    ...
    def GetGconfEntries( self ):
        ...
        self.height = self.gconf.get( "int", "height", 480 )
        ...
        self.useAPT = self.gconf.get( "bool", "use_apt", False )
    ...
    def checksnowMenuFolder( self ):
        if os.path.exists( os.path.join( os.path.expanduser( "~" ), ".snowlinux", "snowMenu", "applications" ) ):
            return True
        try:
            os.makedirs( os.path.join( os.path.expanduser( "~" ), ".snowlinux", "snowMenu", "applications" ) )
            return True
        except:
            pass
    ...
    def add_search_suggestions(self, text):
        ...
        suggestionButton.set_image("/usr/lib/snowlinux/snowMenu/search_engines/google.ico")
        ...
        suggestionButton.set_image("/usr/lib/snowlinux/snowMenu/search_engines/wikipedia.ico")
        self.applicationsBox.add(suggestionButton)
        self.suggestions.append(suggestionButton)
        ...
        self.snowMenuWin.SetPaneColors( [ separator ] )
        separator.show_all()
        ...
        suggestionButton.set_image("/usr/lib/snowlinux/snowMenu/search_engines/dictionary.png")
        self.applicationsBox.add(suggestionButton)
        self.suggestions.append(suggestionButton)
    ...
    def add_apt_filter_results(self, keyword):
        try:
            ...
            command = "cat %(home)s/.snowlinux/snowMenu/apt.cache" % {'home':home}
            ...
            try:
                if keyword == self.searchEntry.get_text() and len(found_packages) > 0:
                    ...
                    self.snowMenuWin.SetPaneColors( [  last_separator ] )
    ...
    def Filter( self, widget, category = None ):
        ...
        if widget == self.searchEntry:
            if self.donotfilterapps:
                ...
                if (not showns and os.path.exists("/usr/lib/snowlinux/snowInstall/icon.svg")):
                    if len(text) >= 3:
                        if self.current_suggestion is not None and self.current_suggestion in text:
                            ...
                            #else:
                            thr = threading.Thread(name="snow-menu-apt-filter", group=None, target=self.add_apt_filter_results, args=([text]), kwargs={})
                            thr.start()
                        else:
    ...
    def searchPopup( self, widget=None, event=None ):
        ...
        img = gtk.Image()
        img.set_from_file('/usr/lib/snowlinux/snowMenu/search_engines/google.ico')
        ...
        img = gtk.Image()
        img.set_from_file('/usr/lib/snowlinux/snowMenu/search_engines/wikipedia.ico')
        menuItem.set_image(img)
        ...
        img = gtk.Image()
        img.set_from_file('/usr/lib/snowlinux/snowMenu/search_engines/dictionary.png')
        menuItem.set_image(img)
    ...
    def add_to_desktop(self, widget, desktopEntry):
        try:
            ...
            import sys, commands
            sys.path.append('/usr/lib/snowlinux/snowMenu/plugins')
    ...
    def add_to_panel(self, widget, desktopEntry):
        import random
        object_name = "snowmenu_"+''.join([random.choice('abcdefghijklmnopqrstuvwxyz0123456789') for x in xrange(8)])
    ...
    def favoritesBuildSpace( self ):
        ...
        self.snowMenuWin.SetPaneColors( [ space ] )
        space.show()
    ...
    def favoritesBuildLauncher( self, location ):
        try:
            ...
            if favButton.appExec:
                ...
                favButton.connect( "clicked", lambda w: self.snowMenuWin.hide() )

                self.snowMenuWin.setTooltip( favButton, favButton.getTooltip() )
    ...
    def buildFavorites( self ):
        try:
            from user import home
            if (not os.path.exists(home + "/.snowlinux/snowMenu/applications.list")):
                os.system("mkdir -p " + home + "/.snowlinux/snowMenu/applications")
                os.system("cp /usr/lib/snowlinux/snowMenu/applications.list " + home + "/.snowlinux/snowMenu/applications.list")

            applicationsFile = open ( os.path.join( os.path.expanduser( "~" ), ".snowlinux", "snowMenu", "applications.list" ), "r" )
    ...
    def favoritesSave( self ):
        try:
            self.checksnowMenuFolder()
            appListFile = open( os.path.join( os.path.expanduser( "~"), ".snowlinux", "snowMenu", "applications.list" ) , "w" )
            ...
            msgDlg = gtk.MessageDialog( None, gtk.DIALOG_MODAL, gtk.MESSAGE_ERROR, gtk.BUTTONS_OK, _("Couldn't save favorites. Check if you have write access to ~/.snowlinux/snowMenu")+"\n(" + e.__str__() + ")" )
            msgDlg.run();
            msgDlg.destroy();
    ...
    def updateBoxes( self, menu_has_changed ):
        ...
        if addedCategories:
            ...
            for item in addedCategories:
                ...
                self.snowMenuWin.setTooltip( item["button"], item["tooltip"] )
            ...
            self.showFiles = True
            if self.showFiles:
                Button = easyButton('user-home', categoryIconSize, [_('Home Folder')], -1, -1)
                Button.connect('clicked', self.ButtonClicked, 'xdg-open {}'.format(home))
                Button.show()
                self.snowMenuWin.setTooltip(Button, _('Open your personal folder'))
                sortedCategoryList.append(Button)

            #LOCK SCREEN
            self.showLockScreen = True
            if self.showLockScreen:
                Button1 = easyButton('system-lock-screen', categoryIconSize, [_('Lock Screen')], -1, -1)
                Button1.connect('clicked', self.ButtonClicked, 'mate-screensaver-command -l')
                Button1.show()
                self.snowMenuWin.setTooltip(Button1, _('Requires password to unlock'))
                sortedCategoryList.append(Button1)

            #LOG OUT
            self.showLogout = True
            if self.showLogout:
                Button2 = easyButton('system-log-out', categoryIconSize, [_('Logout')], -1, -1)
                Button2.connect('clicked', self.ButtonClicked, 'mate-session-save --logout-dialog')
                Button2.show()
                self.snowMenuWin.setTooltip(Button2, _('Log out or switch user'))
                sortedCategoryList.append(Button2)

            #QUIT
            self.showQuit = True
            if (self.showQuit == True):
                Button3 = easyButton('system-shutdown', categoryIconSize, [_('Quit')], -1, -1)
                Button3.connect('clicked', self.ButtonClicked, 'mate-session-save --shutdown-dialog')
                Button3.show()
                self.snowMenuWin.setTooltip(Button3, _("Shutdown, restart, suspend or hibernate"))
                sortedCategoryList.append(Button3)
            ###
            ...
            for item in sortedCategoryList:
                try:
                    ...
                    self.categoriesBox.pack_start(item, False)
        ...
        if addedApplications:
            ...
            for item in addedApplications:
                ...
                if item["button"].appExec:
                    self.snowMenuWin.setTooltip( item["button"], item["button"].getTooltip() )
                    item["button"].connect( "button-press-event", self.menuPopup )
                    item["button"].connect( "focus-in-event", self.scrollItemIntoView )
                    item["button"].connect( "clicked", lambda w: self.snowMenuWin.hide() )
