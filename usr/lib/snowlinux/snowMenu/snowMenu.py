...
if (architecture.find("x86_64") >= 0):
    ...
    libc.prctl(15, 'snowmenu', 0, 0, 0)
else:
    import dl
    if os.path.exists('/lib/libc.so.6'):
        libc = dl.open('/lib/libc.so.6')
        libc.call('prctl', 15, 'snowmenu', 0, 0, 0)
...
class MainWindow( object ):
    ...
    def __init__( self, toggleButton, settings ):
        ...
        builder = Gtk.Builder()
        builder.add_from_file(os.path.join( self.path, "snowMenu.ui" ))
        ...
        plugindir = os.path.join( os.path.expanduser( "~" ), ".snowlinux/snowMenu/plugins" )
            ...
            if plugin != "newpane":
                try:
                    ...
                    if X.pluginclass.__init__.func_code.co_argcount == 1:
                        ...
                        # pass snowMenu and togglebutton instance so that the plugin can use it
...
class MenuWin( object ):
    def __init__( self, applet, iid ):
        self.applet = applet        
        self.settings = Gio.Settings.new("com.snowlinux.snowmenu")
    ...
    def showPreferences( self, action, userdata = None ):
        Execute( os.path.join( PATH, "snowMenuConfig.py" ) )
    ...
    def create_menu(self):
        ...
        action = Gtk.Action("SnowMenuPrefs", _("Preferences"), None, "gtk-preferences")
        action.connect("activate", self.showPreferences)
        action_group.add_action(action)
        action_group.add_action(action)
        action.connect("activate", self.showMenuEditor)
        action_group.add_action(action)
        action = Gtk.Action("SnowMenuReload", _("Reload plugins"), None, "gtk-refresh")
        action.connect("activate", self.mainwin.RegenPlugins)
        action_group.add_action(action)
        action = Gtk.Action("SnowMenuAbout", _("About"), None, "gtk-about")
        action.connect("activate", self.showAboutDialog)
        action_group.add_action(action)
        action_group.set_translation_domain ("snowmenu")
